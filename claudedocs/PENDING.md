...
## いまの状態（2026-08-25 09:20）

走っているものは無い。git は clean、すべてプッシュ済み（`d4082c4`）。テスト402件パス。
...
| 設立日 | 2,447 | 63% | 1,433 |
| 業績予想 | 2,330 | 60% | 1,550 |
| score_complete | 1,941 | — | — |
| 株価と指標が同期 | 3,858 | — | 22（上場廃止） |
| 上場廃止の印 | 21 | — | — |

**加速したいとき**（400件×2＋score_complete再計算、約8時間）:

```bash
bash scripts/accel_backfill.sh
grep -c "OK (" claudedocs/accel_night2.log     # 進捗
```

⚠️ **これはローカルPCから走る。** スリープすると止まる（止まっても取得済みは
スキップするので、同じコマンドで続きから）。夜間の自動ジョブ（毎晩2:00に60件）
だけでも月内には埋まる。

### 次にやるなら

1. **バックフィルの続き**（あと2晩で8〜9割）
...
## 2026-08-24 の作業（コミット済み・**未プッシュ＝未デプロイ**）
